...
//             console.warn("ไม่สามารถเปรียบเทียบวันที่ได้", e);
//           }
...
//       const chartHeight = canvas.height - 30; // เว้นพื้นที่สำหรับแกนเวลา      // วาดแต่ละเส้นตามข้อมูลใน verticalLinesData
//       verticalLinesData.forEach(line => {
//         try {
...
//               try {
//               // ใช้ priceToCoordinate เพื่อแปลงค่าราคาเป็นตำแหน่ง Y ในเวอร์ชั่น 4
//               const priceScale = chart.priceScale('right');
//               if (!priceScale) return;
              
//               // กำหนดตัวแปรไว้เก็บตำแหน่ง Y
//               let y;
//                 // คำนวณตำแหน่ง Y ด้วยค่าสัดส่วนเมื่อ API ไม่รองรับ              try {
//                 // Method 1: ใช้ priceToCoordinate โดยตรง (ถ้ามี)
//                 if (typeof priceScale.priceToCoordinate === 'function') {
//                   y = priceScale.priceToCoordinate(value);
//                   if (y !== null && y !== undefined) {
//                     // พบค่า Y จาก API - ใช้งานได้เลย
//                   } else {
//                     // ถ้า API ส่งค่า null/undefined กลับมา ใช้วิธีการคำนวณเอง
//                     throw new Error("priceToCoordinate returned null");
//                   }
//                 } else {
//                   throw new Error("priceToCoordinate not available");
//                 }
//               } catch (_apiError) { // เพิ่ม underscore เพื่อบอกว่าเราไม่ได้ใช้ตัวแปร
//                 // Method 2: ดึงข้อมูล min-max จาก series แล้วคำนวณเอง
//                 try {
...
//                   // ป้องกันการหารด้วย 0
//                   if (valueRange === 0) {
//                     throw new Error("Value range is 0");
//                   }
...
//                   const topPadding = displayHeight * 0.1;
//                   const bottomPadding = displayHeight * 0.1;
//                   const adjustedHeight = displayHeight - topPadding - bottomPadding;

//                   y = Math.round(topPadding + (valueRatio * adjustedHeight));
//                 } catch (calcError) {
//                   console.warn("Error calculating Y position:", calcError);
//                   // Method 3: ใช้ค่า y กลางของกราฟ (fallback สุดท้าย)
//                   const displayHeight = canvas.height - 30;
//                   y = displayHeight / 2; // ตำแหน่งกลางของกราฟ
//                 }
//               }
...
//                 const labelHeight = 20;                // ตำแหน่งป้ายกำกับ - ปรับตามตำแหน่งของค่า
//                 const labelX = x + 5;
//                 // ใช้ความสูงของกราฟที่เรา define ไว้ก่อนหน้า
//                 const displayHeight = canvas.height - 30; // ลบความสูงของแกน x
...
//     });
//       // ตัวแปรเพื่อป้องกันการเรียก updateVerticalLines ซ้ำๆ ในเวลาเดียวกัน
//     let isUpdating = false;
//     let updateRequested = false;
    
//     // ฟังก์ชันที่ช่วยลดการเรียกซ้ำ
//     const requestUpdate = () => {
//       if (isUpdating) {
...
//         // รอให้การวาดเสร็จก่อนที่จะอนุญาตให้มีการวาดครั้งต่อไป
//         setTimeout(() => {
//           isUpdating = false;
...
//     chart.timeScale().subscribeVisibleLogicalRangeChange(requestUpdate);
    
//     // อัพเดทเมื่อ crosshair เคลื่อนที่ - ลดการวาดซ้ำโดยใช้ throttle
//     let lastCrosshairUpdateTime = 0;
//     chart.subscribeCrosshairMove(() => {
//       const now = Date.now();
//       if (now - lastCrosshairUpdateTime > 100) { // อัพเดทไม่เกิน 10 ครั้งต่อวินาที
//         lastCrosshairUpdateTime = now;
//         requestUpdate();
//       }
//     });
    
//     // อัพเดทเมื่อช่วงเวลาที่มองเห็นเปลี่ยน
//     chart.timeScale().subscribeVisibleTimeRangeChange(requestUpdate);
    
//     // อัพเดทเมื่อใช้เมาส์เลื่อน/ซูม - ใช้ throttle เพื่อลดการทำงานซ้ำ
//     let wheelTimeout;
//     const wheelHandler = () => {
//       if (wheelTimeout) clearTimeout(wheelTimeout);
//       wheelTimeout = setTimeout(requestUpdate, 100);
//     };
...
//     // ไม่ใช้ interval ที่ทำงานตลอดเวลา แต่วาดเฉพาะเมื่อจำเป็น
//     setTimeout(updateVerticalLines, 300);
    
//     // ปรับขนาดเมื่อหน้าจอเปลี่ยน - ใช้ debounce เพื่อลดการทำงานซ้ำ
//     let resizeTimeout;
//     const handleResize = () => {
...
//         chart.resize();
//         updateVerticalLines();
//       }, 100); // รอให้การ resize เสร็จสมบูรณ์ก่อนที่จะอัพเดท
//     };
    
//     // ใช้ ResizeObserver แทนการใช้ window resize event ถ้า browser รองรับ
//     let resizeObserver;
//     if (window.ResizeObserver) {
//       resizeObserver = new ResizeObserver(handleResize);
//       resizeObserver.observe(chartElement);
//     } else {
//       // Fallback สำหรับ browser เก่าๆ
