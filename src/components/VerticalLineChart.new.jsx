...
export const VerticalLineChart = ({
  ...
  useEffect(() => {
    ...
    let processedData;
    if (!hasTimeField) {
      // กรณีไม่มี time field ให้สร้างข้อมูลที่มี time เป็น index
      processedData = data.map((item, index) => ({
        ...item,
        time: index.toString() // ใช้ index แทน time เมื่อไม่มีค่า time (lightweight-charts รับ string ด้วย)
      }));
      
      // แสดง log เพื่อแจ้งเตือน
      console.info("ข้อมูลไม่มี time field - ใช้ index แทน");
    } else {
    ...
    if (verticalLines && Array.isArray(verticalLines) && verticalLines.length > 0) {
      ...
      verticalLinesData = verticalLines.map(line => {
        ...
        if (line.index !== undefined && line.index < processedData.length) {
          ...
          // ในกรณีที่ไม่มี time ในข้อมูล แต่กำหนด time มา ให้แปลง time เป็น index
    ...
    const updateVerticalLines = () => {
      ...
      verticalLinesData.forEach(line => {
        // หาพิกัด x จากค่าเวลาหรือ index
        let x;
        
        if (hasTimeField) {
          // กรณีที่มี time field ใช้ timeToCoordinate
          x = timeScale.timeToCoordinate(line.time);
        } else {
          // กรณีที่ไม่มี time field ใช้ coordinate ตาม index
          // ดึงช่วงข้อมูลที่กำลังแสดง
          const visibleLogicalRange = timeScale.getVisibleLogicalRange();
          if (!visibleLogicalRange) return;
          
          // ตรวจสอบว่า index อยู่ในช่วงที่แสดงหรือไม่
          const lineIndex = parseInt(line.time);
          if (lineIndex < visibleLogicalRange.from || lineIndex > visibleLogicalRange.to) {
            // หากเส้นอยู่นอกช่วง ไม่ต้องแสดง
            return;
          }
          
          // คำนวณตำแหน่ง x ตาม index เทียบกับช่วงที่แสดง
          const logicalRange = visibleLogicalRange.to - visibleLogicalRange.from;
          const indexRatio = (lineIndex - visibleLogicalRange.from) / logicalRange;
          
          // ปรับให้อยู่ในพื้นที่กราฟ - เผื่อระยะขอบ
          const chartLeftPadding = 60; // พื้นที่ด้านซ้ายของกราฟ (บริเวณแกนราคา)
          const chartRightPadding = 20; // พื้นที่ด้านขวาของกราฟ
          const chartWidth = canvas.width - chartLeftPadding - chartRightPadding;
          
          // คำนวณตำแหน่ง pixel
          x = Math.round(chartLeftPadding + (indexRatio * chartWidth));
        }
        ...
        if (line.label.show) {
          ...
          if (y !== null) {
            ...
            const labelX = x + 5;
            const labelY = y < chartHeight / 2 ? y + 10 : y - labelHeight - 5;
            
            // วาดกรอบมน (ใช้วิธีที่ compatible กับทุกเบราว์เซอร์)
            ctx.beginPath();
            const radius = 4;
